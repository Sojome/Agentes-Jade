 ...
 */
//-gui -port 1098 Maestro:Tareaprotocolos.SubscriptionReceptor;Representante:Tareaprotocolos.SubscriptionEmisor
public class SubscriptionReceptor extends Agent {
    ...
    protected void setup() {
        System.out.println(this.getLocalName() + ": Esperando tareas...");
    ...
    private class EnviarSemanal extends TickerBehaviour {
        ...
        public void onTick() {
            ...
            for (Subscription suscripcion : SubscriptionReceptor.this.suscripciones) {
    ...
    private class HacerSuscripcion extends SubscriptionResponder {
        ...
        protected ACLMessage handleSubscription(ACLMessage propuesta)
            ...
            System.out.printf("%s: Tarea recibido de %s.\n",
                    SubscriptionReceptor.this.getLocalName(), propuesta.getSender().getLocalName());
            System.out.printf("%s: La propuesta es: %s.\n",
                    SubscriptionReceptor.this.getLocalName(), propuesta.getContent());

            //Comprueba los datos de la propuesta
            if (SubscriptionReceptor.this.compruebaMensaje(propuesta.getContent())) {
                ...
                try {
                    ...
                    System.out.println(SubscriptionReceptor.this.getLocalName() + ": Error en el registro de la suscripción.");
                }
        ...
        protected ACLMessage handleCancel(ACLMessage cancelacion) {
            System.out.printf("%s: CANCEL recibido de %s.\n",
                    SubscriptionReceptor.this.getLocalName(), cancelacion.getSender().getLocalName());

            try {
                //El SubscriptionManager elimina del registro la suscripcion
                this.mySubscriptionManager.deregister(this.suscripcion);
            } catch (Exception e) {
                System.out.println(SubscriptionReceptor.this.getLocalName() + ": Error en el desregistro de la suscripción.");
            }
